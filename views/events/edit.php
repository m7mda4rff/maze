 ...
 * صفحة تعديل الفعالية
 * 
 * تستخدم لتعديل بيانات فعالية موجودة
 */
...
// تعيين البيانات الافتراضية من الفعالية المحملة
$formData = [
    'title' => $eventObj->getTitle(),
    'customer_id' => $eventObj->getCustomerId(),
    'event_type_id' => $eventObj->getEventTypeId(),
    'date' => $eventObj->getDate(),
    'start_time' => $eventObj->getStartTime(),
    'end_time' => $eventObj->getEndTime(),
    'location' => $eventObj->getLocation(),
    'guest_count' => $eventObj->getGuestCount(),
    'package_details' => $eventObj->getPackageDetails(),
    'total_package_cost' => $eventObj->getTotalPackageCost(),
    'status_id' => $eventObj->getStatusId(),
    'additional_notes' => $eventObj->getAdditionalNotes()
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!validateCSRFToken($_POST['csrf_token'])) {
        ...
        if (!empty($formData['date']) && !empty($formData['start_time']) && !empty($formData['end_time'])) {
            ...
            // التحقق من تعارض المواعيد مع استثناء الفعالية الحالية
        ...
        // إذا لم تكن هناك أخطاء، قم بتحديث الفعالية
        if (empty($errors)) {
            // تحديث الفعالية
            $result = $eventObj->update($eventId, $formData);
            
            if ($result) {
                setSystemMessage('success', 'تم تحديث الفعالية بنجاح');
                redirect(BASE_URL . '/public/views/events/view.php?id=' . $eventId);
            } else {
                $errors[] = 'حدث خطأ أثناء تحديث الفعالية. يرجى المحاولة مرة أخرى.';
            }
        }
    }
}
...
// إعداد عنوان الصفحة
$pageTitle = 'تعديل الفعالية: ' . $eventObj->getTitle();
...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div>
            <a href="view.php?id=<?php echo $eventId; ?>" class="btn btn-info">
                <i class="fas fa-eye"></i> عرض الفعالية
            </a>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form method="post" action="edit.php?id=<?php echo $eventId; ?>">
                ...
                <div class="mt-4">
                    ...
                    <a href="view.php?id=<?php echo $eventId; ?>" class="btn btn-secondary">
